...
class WebsiteController extends Controller
{
    ...
    { 
        // MellyMunchies
        $this->theme = env('ACTIVE_THEME','MellyMunchies');
    ...
    return view("theme.MellyMunchies.about");
    }
    public function Products(Request $request){   
    return view("theme.MellyMunchies.products");
    }
    public function contact(Request $request){   
    return view("theme.MellyMunchies.contact");
    }
    public function gift(Request $request){   
    return view("theme.MellyMunchies.gift");
    }
    ...
    {
        $starting_value = intval( substr($request->starting_value, 1));
        $ending_value = intval( substr($request->ending_value, 1));
        $products = Product::whereBetween('price', [$starting_value, $ending_value])->where('is_gift',0)
           ->get();
        $data = "";
        foreach ($products as $item) {
            $productsTranslation = ProductTranslation::where('product_id', $item->id)->first();
            $data .= '<div class="col-12 col-lg-4 mt-5 p-0">
                <a href="' . url('product/' . $item->slug) . '">
                    <div class="product-card gift-card">
                        <img src="http://localhost/mellymunchies/storage/app/' . $item->image->file_path . '" class="img-fluid" alt="image" />
                        <div class="our-product_content">
                            ...
                            <div class="align-items-center justify-content-between d-flex">
                                ...
                                <div class="theme-group">
                                    <a href="' . url('add_to_cart/' . $item->id) . '" class="theme-btn btn add_to_cart" data-type="' . $item->type . '">add to cart</a>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    public function gift_filter(Request $request)
    {
        $starting_value = intval( substr($request->starting_value, 1));
        $ending_value = intval( substr($request->ending_value, 1));
        $products = Product::whereBetween('price', [$starting_value, $ending_value])->where('is_gift',0)
